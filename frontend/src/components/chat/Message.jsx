  ...
  return (
    ...
    >
      <motion.div
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        className={`relative max-w-[80%] md:max-w-[60%] group`}
      >
        <div className="flex items-end gap-2 relative">
          {!ownMessage && (
            <div
              className={`flex flex-col gap-2 absolute -left-10 top-0 opacity-0 group-hover:opacity-100 transition-opacity`}
            >
              <button
                onClick={() => generateAIResponse(message, "reply")}
                className="p-2 bg-indigo-100 hover:bg-indigo-200 rounded-full text-indigo-600 transition-colors"
          ...
          <div
            className={`inline-block rounded-2xl px-4 py-2 shadow-sm break-words
        ${
          ownMessage
